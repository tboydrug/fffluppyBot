...
client = commands.Bot(command_prefix=prefix, help_command=None,
                      intents=disnake.Intents.all(), test_guilds=[1057408057426067526])

connection = sqlite3.connect('server.db')
cursor = connection.cursor()

ROLES_TO_CHANGE = [
    {'role_id': 1102249945207160903, 'colors': [
        '#020202', '#fa0000', '#d38f4c', '#f8fa00', '#4caf2d', '#00ffe9', '#374ac0', '#d666cc']}
]
    ...
    for guild in client.guilds:
        for member in guild.members:
            ...
            if cursor.execute(f"SELECT id FROM users WHERE id = {member.id}").fetchone() is None:
                ...
                cursor.execute(f"UPDATE users SET name = ? WHERE id = {
                               member.id}", (username,))

    connection.commit()

    if await remove_expired_roles.is_running():
...
async def add_coins(ctx, member: disnake.Member, count: int):
    ...
    cursor.execute(
        f"UPDATE users SET coins = coins + '{count}' WHERE id = '{user_id}'")
    connection.commit()
...
async def create_role(guild, role_name, duration, colour):
    if role_name != cursor.execute("SELECT role_name FROM roles WHERE role_name = ?", (role_name,)).fetchone():
        ...
        cursor.execute(f"INSERT INTO roles (role_id, role_name, color, created_at) VALUES ({
                       role.id}, '{role_name}', '{colour}', {created_at + duration})")
        connection.commit()
...
async def купить_роль(ctx, name: str, colour: str = '020202'):
    ...
    if balance >= 5000:
        ...
        await create_role(ctx.guild, name, 2592000, disnake.Colour(int(colour, 16)))
        role = disnake.utils.get(ctx.guild.roles, name=name)
        existing_role = disnake.utils.get(
            ctx.guild.roles, name='Керринг машина')
        if existing_role:
        ...
        cursor.execute(
            f"UPDATE users SET coins = coins - 5000 WHERE id = '{user_id}'")
        connection.commit()
...
async def role_button(ctx, role_id: int):
    ...
    if role is not None:
        ...
        button = disnake.ui.Button(style=disnake.ButtonStyle.green,
                                   label='НАЖМИ НА МЕНЯ', custom_id=f'role_button_{role_id}')
...
async def on_member_update(before, after):
    ...
    after_without_roles = [
        role for role in after.roles if role != after.guild.default_role]
    if not after_without_roles:
        await after.add_roles(role_to_remove)
    else:
        before_with_role_to_remove = [
            role for role in before.roles if role == role_to_remove]
...
async def on_member_join(member):
    if member.discriminator is not None:
        ...
        username = member.name
    if cursor.execute(f"SELECT id FROM users WHERE id = {member.id}").fetchone() is None:
        cursor.execute("INSERT INTO users (name, id, tw_id, coins, rep, rank, points) VALUES (?, ?, ?, ?, ?, ?, ?)",
                       (username, member.id, 'NULL', 0, 0, 0, 0))
    else:
        pass
...
async def profile(ctx):
    ...
    rank_img = Image.open(ranks_img[cursor.execute(
        "SELECT rank FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0]])
    ...
    # Twitch name
    text = f"""{cursor.execute("SELECT tw_id FROM users WHERE id = {}".format(
        ctx.author.id)).fetchone()[0]}"""
    # Name
    text1 = f"{ctx.author.name}"
    # Level
    text2 = f"""RANK: {rank[cursor.execute(
        "SELECT rank FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0]]}"""
    # experience
    text3 = f"""POINTS: {cursor.execute(
        "SELECT points FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0]}"""
    # reputation
    text4 = f"""REP: {cursor.execute(
        "SELECT rep FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0]}"""
    # balance
    text5 = f"""FLUPYCOIN: {cursor.execute(
        "SELECT coins FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0]}"""

    # Draw text TW_NAME
    text_width, text_height = draw.textsize(text1, font=font)
    draw.text(((img.width - text_width) // 2, img.height -
              text_height - 200), text, font=font, fill=(0, 0, 0))
    # Draw text NAME
    text_width, text_height = draw.textsize(text1, font=font)
    draw.text(((img.width - text_width) // 2, img.height -
              text_height - 145), text1, font=font, fill=(0, 0, 0))
    # Draw text RANK
    ...
    if channel is not None and bool(rewards):
        for reward in rewards['data']:
            ...
            if cursor.execute(f"SELECT id FROM users WHERE name = '{reward['user_input']}'").fetchone():
                status = "FULFILLED"
                cursor.execute(f"UPDATE users SET coins = coins + {
                               reward['reward']['cost']} WHERE name = '{reward['user_input']}'")
                connection.commit()
